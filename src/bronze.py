...
def load_bronze(path,engine):
    ...
    print(f"{table_name}.csv loaded : {len(df)} lignes")
    ...
    print("loading done")
    print(f"{table_name}|nombre de colonnes:{len(df.columns)}|{len(df)} lignes| {elapsed:.2f} s|date_ingest:{date}")
